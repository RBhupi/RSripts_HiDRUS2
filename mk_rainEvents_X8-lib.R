...
# The 'X2'and 'X4' means ts was repeated two/four times
#===========================================================================
...
ofileName<-"RainLib-X4_2008-14_MLB"  #no extension

#--------- Some initialisation settings----------------#
window <- 240           #number of point in sampled ts
cut_off=1             #mean area rain mm/hr
...
id_number=40000  #start identity number for the event
case_number=0 # actual number of the event.
extd_mean<-10  # only event for more rain than value will be  selected
...
window2 <- window/8  #a window for searching events
...
for (i in 1:(prms.dim[1]-window2))
{
    ...
    if(isFirst)   #if this is the first valid 'ts' we have found then save it.
    {
        ...
        df_out <- rbind(prms[i:(i+window2-1),], prms[i:(i+window2-1),], prms[i:(i+window2-1),], prms[i:(i+window2-1),], prms[i:(i+window2-1),], prms[i:(i+window2-1),], prms[i:(i+window2-1),], prms[i:(i+window2-1),], deparse.level = 1)
