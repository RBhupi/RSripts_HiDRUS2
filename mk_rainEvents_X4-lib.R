...
id_number=20000  #start identity number for the event
...
window2 <- window/4  #a window for searching events
...
for (i in 1:(prms.dim[1]-window2))
{
    ...
    if(isFirst)   #if this is the first valid 'ts' we have found then save it.
    {
        ...
        df_out <- rbind(prms[i:(i+window2-1),], prms[i:(i+window2-1),], prms[i:(i+window2-1),], prms[i:(i+window2-1),],deparse.level = 1)
