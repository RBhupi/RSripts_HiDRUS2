...
nc_calendar <- function(fName){
    ncFile <- nc_open(gcm_rainFile)
    return(x$dim$time$calendar)
}
...
#s_date <- as.Date("2040-01-01", format = "%Y-%m-%d")
#e_date <- as.Date("2049-12-31", format = "%Y-%m-%d")

nc_cal <- nc_calendar(gcm_rainFile)
s_date <- as.PCICt("2040-01-01", cal = nc_cal) #change it to model calendar
e_date <- as.PCICt("2049-12-30", cal = nc_cal)
...
gcm_date <- round(as.PCICt((gcm_time-1)*seconds.per.day, origin="1800-01-01", cal = nc_cal), "days")
...
gcmslp_date<- round(as.PCICt((gcmslp_time-1)*seconds.per.day, origin="1800-01-01", cal = nc_cal), "days")
...
outFilePrefix <- paste("h2xprm", city,
                       paste(as.numeric(format(s_date, "%Y")), as.numeric(format(e_date, "%Y")), sep="-"),
                       model, scenario, sep = "_")
...
for(ens in ens_id){
    ...
    s_time<-as.double(as.PCICt(s_date, tz="UTC"))
